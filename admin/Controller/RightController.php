...
class RightController extends AppController {
    ...
    public $name = 'Right';
    public $uses = array('Right', 'RightCategory');
	...
	*/
    public function index() {
		$this->paginate = array('limit'=>15, 'order'=>'Right.id DESC');
		$Right = $this->paginate('Right');
	...
	*/
    public function add() {
        ...
        if (!empty($this->request->data)) {	
			...
			if(empty($data['Right']['right_category_id'])){
				$this->Session->setFlash(__('Chưa chọn danh mục  !', true));
			}else{
				if ($this->Right->save($data)){
	...
	*/
	public function edit($id = null)
	{
		...
		if (!$id && empty($this->request->data)) {
            $this->redirect(array('controller'=>'Right', 'action'=>'index'));
        }else if (!empty($this->request->data)) {
            $data = $this->request->data;
            if(empty($data['Right']['right_category_id'])){
                $this->Session->setFlash(__('Chưa chọn danh mục  !', true));
            }else{
                if ($this->Right->save($data['Right'])){
	...
	*/
	public function delete_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Right->delete($condition)){
	...
	*/
	public function active($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->Right->save(array('Right'=>array('status'=>1, 'id'=>$id)));
	...
	*/
	public function active_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Right->updateAll(array('status'=>1), array('Right.id'=>$condition))){
	...
	*/
	public function close($id = null)
	{
		$this->Session->setFlash(__('', true));
		if (!empty($id)){
            $this->Right->save(array('Right'=>array('status'=>0, 'id'=>$id)));
	...
	*/
	public function close_all($data_action)
	{
		...
		if(!empty($data_action)){
			...
			if($this->Right->updateAll(array('status'=>0), array('Right.id'=>$condition))){
	...
	*/
	public function search()
	{
		if($this->request->is('POST')){
			...
			$conditions = array();
			if(!empty($keyword)){
				$conditions[] = array('Right.name like "%'.$keyword.'%"');
			}
			if(!empty($category_id)){
				$conditions[] = array('Right.Right_category_id'=>$category_id);
			}
			//pr($conditions); die;
			$this->paginate = array(
					'conditions'=>$conditions,
					'limit'=>15,
					'order'=>'Right.id DESC, Right.modified DESC'
			);
			$Right = $this->paginate('Right');
        	$this->set('Right', $Right);
            $list_cat = $this->get_category('RightCategory');
